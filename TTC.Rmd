...
train_data_comp$cost_category <- as.factor(train_data_comp$cost_category)
train_ind <- createDataPartition(train_data_comp$cost_category, p = .8, 
                                  list = FALSE, times = 1)
train_set <- train_data_comp[train_ind,]
test_set <- train_data_comp[-train_ind,]
...
head(predict(mda_model, newdata=test_data_comp, type = "prob"))

final_table <- predict(mda_model, newdata=test_data_comp, type = "prob")

final_table <- final_table %>%
  add_column(Tour_ID = test_data_comp$Tour_ID,
             .before = "High Cost") 
