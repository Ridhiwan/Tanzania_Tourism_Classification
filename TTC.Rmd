...
LogLoss(mtcars_glm_model$finalModel$fitted.values,as.numeric(mtcars_test$vs))
...
mda_model <- train(form = cost_category ~ info_source + main_activity,data = train_set,trControl = trainControl(method = "cv", number = 5, classProbs = TRUE,  summaryFunction=mnLogLoss),
