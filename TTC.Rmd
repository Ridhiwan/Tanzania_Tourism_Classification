...
test_data2$country <- factor(test_data2$country,levels = c("ITALY","FRANCE","GERMANY","UNITED STATES OF AMERICA","SOUTH AFRICA", "UNITED KINGDOM", "KENYA", "ZAMBIA", "AUSTRALIA","CANADA","ZIMBABWE","UGANDA","SPAIN","INDIA","NETHERLANDS","SWIZERLAND","BELGIUM","SWEDEN","BURUNDI","CHINA", "UAE"))
test_data2$country <- `levels<-`(addNA(test_data2$country), c(levels(test_data2$country),"OTHERS"))
str(test_data2)
```

  We need to add the country column to our data but it has too many levels if factorized. We will device a way to factorize it with a few levels before cheking its importance as a variable in the following steps.
```{r}
country_df <- as.data.frame(table(train_data2$country))
...
train_data2$country <- factor(train_data2$country,levels = c("ITALY","FRANCE","GERMANY","UNITED STATES OF AMERICA","SOUTH AFRICA", "UNITED KINGDOM", "KENYA", "ZAMBIA", "AUSTRALIA","CANADA","ZIMBABWE","UGANDA","SPAIN","INDIA","NETHERLANDS","SWIZERLAND","BELGIUM","SWEDEN","BURUNDI","CHINA", "UAE"))
train_data2$country <- `levels<-`(addNA(train_data2$country), c(levels(train_data2$country),"OTHERS"))
str(train_data2)
...
```{r}
# Feature selection
corr <- lapply(train_data2,as.numeric)
corMat <- as.data.frame(corr)
highcor <- findCorrelation(corMat,cutoff = 0.75 , exact = FALSE, names = FALSE)
highcor
...
```{r}
train_data2 <- train_data2 %>% add_column(Tour_ID = train_data1$Tour_ID)
test_data2 <- test_data2 %>% add_column(Tour_ID = test_data1$Tour_ID)
...
train_ind <- createDataPartition(train_data2$cost_category, p = .8, 
                                  list = FALSE, times = 1)


train_set <- train_data2[train_ind,]
test_set <- train_data2[-train_ind,]
...
mda_model <- train(form = cost_category ~ info_source + package_transport_tz + package_food + main_activity ,data = train_set, trControl = ctrl,
  method = "svmRadial", metric = "logLoss")
...
head(predict(mda_model, newdata=test_data2, type = "prob"))

final_table <- predict(mda_model, newdata=test_data2, type = "prob")
...
final_table <- final_table %>%
  add_column(Tour_ID = test_data2$Tour_ID,
             .before = "High Cost") 
