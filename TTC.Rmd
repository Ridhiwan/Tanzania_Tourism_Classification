...
impute_train_data <- amelia(train_data_f, m=5, idvars = c("Tour_ID","package_accomodation","tour_arrangement","package_sightseeing","package_guided_tour","package_transport_tz","package_transport_int","package_food","total_female","total_male","first_trip_tz","age_group","country","night_mainland","night_zanzibar","info_source","package_insurance","purpose" ), noms = c(4,8), ords = "cost_category")
...
mda_model <- train(form = cost_category ~  main_activity + package_transport_int + package_accomodation + tour_arrangement + age_group + purpose + night_mainland + first_trip_tz ,data = train_set, trControl = ctrl,
...
ctrl<-trainControl(method = "repeatedcv", number=10, repeats = 10, selectionFunction = "oneSE", classProbs = TRUE,  summaryFunction=mnLogLoss)
#grid<-expand.grid(parallel=FALSE, link= "loge") 


#nn_model <- nnet::multinom(cost_category ~ ., data= train_set)
mda_model <- train(form = cost_category ~ main_activity + package_transport_int + package_accomodation + tour_arrangement + age_group + purpose + night_mainland + first_trip_tz  + package_transport_tz + info_source + package_insurance + package_food ,data = train_set, trControl = ctrl,
  method = "rf", metric = "logLoss", na.action=na.pass)
